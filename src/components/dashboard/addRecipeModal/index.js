...
export default function AddRecipeModal({open, setOpen}) {
  ...
  const handleSubmit = () => {
    if (formData.title == "") {
      ...
      requestData.ingredients = JSON.stringify(formData.ingredients)
      requestData.steps = JSON.stringify(formData.steps)

      dispatch(createRecipe(requestData)).then((res) => {
        ...
        <FormControl fullWidth>
          ...
          {formData.imagePreview && (
            <img
              ...
              alt="Preview"
              style={{maxWidth: "25rem", marginTop: "10px"}}
            />
          )}
          <FormControlLabel
            control={
